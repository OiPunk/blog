---
title: 'Nginx与Tengine详解'
date: '2020-06-12'
tags: ['Nginx']
draft: false
summary: '什么是服务端负载均衡？Nginx与Tengine的解析'
---
...
![image-20200103154530929](/static/images/blog/20200103154530929.png)
...
```plain
工作模式与连接数上限events{参考事件模型，use [ kqueue | rtsig | epoll | /dev/poll | select | poll ]; epoll模型是Linux 2.6以上版本内核中的高性能网络I/O模型，如果跑在FreeBSD上面，就用kqueue模型。use epoll;单个进程最大连接数（最大连接数=连接数*进程数）worker_connections 65535;}
```
...
```plain
日志相关access_log  "pipe:rollback logs/host.access_log interval=1d baknum=7 maxsize=2G"  main;
```
...
![1562927031929](/static/images/blog/C:%5CUsers%5CAdministrator%5CAppData%5CRoaming%5CTypora%5Ctypora-user-images%5C1562927031929.png)
...
```plain
location  {	   deny  IP /IP段	   deny  192.168.1.109;	   allow 192.168.1.0/24;192.168.0.0/16;192.0.0.0/8	}
```
...
```plain
location ~ (.*)\.avi$ {         auth_basic  "closed site";         auth_basic_user_file conf/users;}
```

### **httpd-tools**

```plain
yum install httpdhtpasswd -c -d /usr/local/users zhangyang
```
...
![image-20200103154711795](/static/images/blog/image-20200103154711795.png)

#### 经典的反向代理结构

![image-20200103154652173](/static/images/blog/image-20200103154652173.png)
...
```plain
memcached -d -m 128 -u root -l 192.168.43.151 -p 11211 -c 256 -P /tmp/memcached.pidmemcached-tool 192.168.2.51:11211参数解释：	-d:后台启动服务	-m:缓存大小	-p：端口	-l:IP	-P:服务器启动后的系统进程ID，存储的文件	-u:服务器启动是以哪个用户名作为管理用户
```

#### Nginx配置

```plain
upstream tomcat{        server 192.168.2.52:8080;        server 192.168.2.53:8080;      }        location /tomcat {        proxy_pass http://tomcat/;      }
```
...
```plain
chkconfig --listchkconfig --add nginxchkconfig nginx on
```
...
```plain
location / {proxy_pass http://192.168.150.11:803;        }         location ~ .*\.(gif|jpg|jpeg|png|bmp|swf|html|htm|css|js)$ {          root  /var/data/www1/;            }
```
