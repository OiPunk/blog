...
create table enum\_test(e enum(‘fish’,’apple’,’dog’) not null);

insert into enum\_test(e) values(‘fish’),(‘dog’),(‘apple’);

select e+0 from enum\_test;
...
人们经常使用varchar(15)来存储ip地址，然而，它的本质是32位无符号整数不是字符串，可以使用INET\_ATON()和INET\_NTOA函数在这两种表示方法之间转换

案例：

select inet\_aton(‘1.1.1.1’)

select inet\_ntoa(16843009)
...
select id fom url where url=”” and url\_crc=CRC32(“”)
...
select \* from actor inner join film\_actor using(actor\_id) inner join film using(film\_id) where film.title=’Academy Dinosaur’;
