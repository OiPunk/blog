...
```sh
手写客户端负载均衡1、知道自己的请求目的地（虚拟主机名，默认是spring.application.name）2、获取所有服务端地址列表（也就是注册表）。3、选出一个地址，找到虚拟主机名对应的ip、port（将虚拟主机名 对应到 ip和port上）。4、发起实际请求(最朴素的请求)。
```
...
官网首页：[https://github.com/Netflix/ribbon](https://github.com/Netflix/ribbon)
...
```sh
ribbon.eureka.enabled=falseribbon.listOfServers=localhost:80,localhost:81
```
